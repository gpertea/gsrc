...
mgap=$(awk "BEGIN {printf \"%d\", $mgap * 0.2 + $mgap - $rlen * 2}")
cseg=$(awk "BEGIN {printf \"%d\", $rlen * 0.85}")

echo "mgap=$mgap" >> $outdir/lib_info.txt
echo "cseg=$cseg" >> $outdir/lib_info.txt
halfthreads=$(awk "BEGIN {printf \"%d\", $threads / 2}")
#if [ ! -L star_$lib ]; then
# ln -s $d .
#fi
...
machine=$(uname -n)
cd $outdir/_1
star_options="--genomeDir $starIdx --genomeLoad LoadAndKeep --runThreadN $halfthreads \
 --alignIntronMax 1 --alignIntronMin 2 --outFileNamePrefix aln_1. \
 --outSAMprimaryFlag AllBestScore --outSAMreadID Number --outSAMorder PairedKeepInputOrder \
...
STAR $star_options \
 --readFilesIn $src/$WGS/$lib"_1.fastq" &> star.log &
pid1=$!
sleep 30
cd ../_2

STAR $star_options \
 --readFilesIn $src/$WGS/$lib"_2.fastq" &> star.log &
pid2=$!
