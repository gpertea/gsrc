...
# chrname => [ tdata1, tdata2,  ... ]
# tdata is [ strand, exonlist ]
# exonlist is [ [e1start, e1end], [e2start, e2end], ... ]
if($guided) {
    ...
    if($rname ne $lastref) { # start a new chromosome
	    ...
	    if($guided) { # adjust currentstart and currentend based on the guiding transfrags
	      ...
	      if($ng_start<$ng && $guides[$ng_start][1][0][0]<$pos) { 
	        ...
	        $ng_end=$ng_start;
	        while($ng_end+1<$ng && $guides[$ng_end+1][1][1][0]<=$pos) {
	          $ng_end++;
...
### introduced this update to hopefully speed up things
...
=begin
	    ...
	    print STDERR "Graph $s $g:\n";
	    for(my $t=0;$t<$trnumber[$s][$g];$t++) {
	      print STDERR $t,": ",unpack("b*",$no2tr[$s][$g][$t])," ",$transfrag[$s][$g][$t],"\n";
	    }
...
=cut
	    ...
	    update_nodes($s,$g,\@trnumber,\@no2tr,\@tr2no,$transfrag[$s][$g],$no2gnode[$s][$g],$graphno[$s][$g],\@compatible,\@trnode,\@in,\@out,\@sumin,\@sumout); # compute transfrag compatibility, node abundances

=begin

	    print STDERR "Transfrags and coverages for nodes:\n";

	    for(my $i=0;$i<$graphno[$s][$g];$i++) {
	      print STDERR "Node $i= In:";
	      my $nin=scalar(@{$in[$i]});
	      for(my $t=0;$t<$nin;$t++) {
		print STDERR " ",$in[$i][$t],"(",$transfrag[$s][$g][$in[$i][$t]],")";
	      }
	      print STDERR " Out:";
	      my $nout=scalar(@{$out[$i]});
	      for(my $t=0;$t<$nout;$t++) {
		print STDERR " ",$out[$i][$t],"(",$transfrag[$s][$g][$out[$i][$t]],")";
	      }	      
	      print STDERR "\n";
...
=cut
	    ...
	    $geneno=find_transcripts($graphno[$s][$g],$no2tr[$s][$g],$tr2no[$s][$g],$transfrag[$s][$g],$no2gnode[$s][$g],\@compatible,\@trnode,\@in,\@out,\@sumin,\@sumout,$refname,$geneno,$label,$s,$readthr,$mintranscriptlen);
...
sub find_transcripts {
    my ($gno,$no2tr,$tr2no,$transfrag,$no2gnode,$compatible,$trnode,$in,$out,$sumin,$sumout,$refname,$geneno,$label,$strand,$readthr,$mintranscriptlen)=@_;
    ...
    for(my $i=0;$i<$gno;$i++) {
      ...
      if($i) { 
	    ...
	    #print STDERR "Nodecov[$i]=",$nodecov[$i]," sumin=",$$sumin[$i]," sumout=",$$sumout[$i],"\n";

	    my $nr=scalar(@{$$out[$i]});
	    my $nl=scalar(@{$$in[$i]});

	    $$sumin[$i]=0 unless $$sumin[$i];
	    $$sumout[$i]=0 unless $$sumout[$i];

	    if(!$$sumin[$i] && $$inode[4][0][2]) { # node should have parents but there are no junctions supporting it
	      if($$sumout[$i]) { $$sumin[$i]=$$sumout[$i];}
	      else { $$sumin[$i]=$$inode[7];}
	      my $t=$$tr2no{$$inode[8]}; # transfrag of current node
	      push(@{$$in[$i]},$t);
	      $intrcov[$t]=$$sumin[$i];
	    }

	    if(!$$sumout[$i] && scalar(@{$$inode[3]})) { # node should have children but there are no junctions supporting it
	      if($$sumin[$i]) { $$sumout[$i]=$$sumin[$i];}
	      else { $$sumout[$i]=$$inode[7];}
	      my $t=$$tr2no{$$inode[8]}; # transfrag of current node
	      push(@{$$out[$i]},$t);
	      $outtrcov[$t]=$$sumout[$i];
	    }
	    ...
	    if($$sumin[$i]>$$sumout[$i]) { # more transcripts enter than exit
	      if(scalar(@{$$inode[3]})) { # node i has children
		my $t=$$tr2no{$$inode[8]}; # transfrag of current node
		push(@{$$out[$i]},$t);
		$outtrcov[$t]=$$sumin[$i]-$$sumout[$i];
	      }
	    }
	    elsif($$sumin[$i]<$$sumout[$i]) { # more transcripts exit than enter
	      if($$inode[4][0][2]) { # source is not the parent of node i
		my $t=$$tr2no{$$inode[8]}; # transfrag of current node
		push(@{$$in[$i]},$t);
		$intrcov[$t]=$$sumout[$i]-$$sumin[$i];
	      }
	      $$sumin[$i]=$$sumout[$i];
	    }
	    
	    if($$sumin[$i]) { $$sumout[$i]=1/$$sumin[$i];} # sumout now keeps 1% of the node

	    for(my $t=0;$t<$nl;$t++) {
		$intrcov[$$in[$i][$t]]=$$transfrag[$$in[$i][$t]];
	    }


	    for(my $t=0;$t<$nr;$t++) {
		$outtrcov[$$out[$i][$t]]=$$transfrag[$$out[$i][$t]];
	    }
	}
    }
    ...
    if($nodecov[$maxi]>=$readthr) { $geneno=parse_trf(0,$ne,$maxi,$no2tr,$tr2no,$transfrag,$no2gnode,$compatible,$trnode,$in,$out,$sumout,\@nodecov,\@intrcov,\@outtrcov,\%trforder,\%seenedge,$refname,$geneno,1,$label,$strand,$readthr,$mintranscriptlen,$gno);}
...
sub max_compon_size {
  my ($set,$result,$compatible,$cov,$computed)=@_;
  ...
  for(my $i=0;$i<$n;$i++) {
    ...
    if($#agree>-1) {
      ...
      if(!$agreesize) {
	$agreesize=max_compon_size(\@agree,\@agres,$compatible,$cov,$computed);
...
sub parse_trf {
    my ($ve,$ne,$maxi,$no2tr,$tr2no,$transfrag,$no2gnode,$compatible,$trnode,$in,$out,$oneperc,$nodecov,$intrcov,$outtrcov,$trforder,$seenedge,$refname,$geneno,$trno,$label,$strand,$readthr,$mintranscriptlen,$gno)=@_;

    #print STDERR "Start parse with cov[$maxi]=",$$nodecov[$maxi],"\n";
    ...
    my %computed;
    $ve=back_to_source_path($maxi,\@path,\%pathincov,\%pathoutcov,\$pathpat,\@transcripts,\%istranscript,$in,$out,$intrcov,$outtrcov,$trnode,$compatible,$ve,$seenedge,$no2gnode,$tr2no,$no2tr,$gno,\%computed);
	...
	add_tr2vec($transcripts[$t],-1,-1,$trnode,\@tmp);
      ...
      $ve=fwd_to_sink_path($maxi,\@path,\%pathincov,\%pathoutcov,\$pathpat,\@transcripts,\%istranscript,$in,$out,$intrcov,$outtrcov,$trnode,$compatible,$ve,$seenedge,$no2gnode,$tr2no,$no2tr,$gno,\%computed);    
	...
	for(my $i=0;$i<=$#path;$i++) {
	  if($i) {
	    my $nl=scalar(@{$$in[$path[$i]]});
	    for(my $t=0;$t<$nl;$t++) {
	      if(!(defined $istranscript{$$in[$path[$i]][$t]}) && (($pathpat & $$no2tr[$$in[$path[$i]][$t]]) eq $$no2tr[$$in[$path[$i]][$t]])) { # transcript is compatible to path
		#print STDERR "Transcript ",$$in[$path[$i]][$t]," is compatible with path\n";
		$istranscript{$$in[$path[$i]][$t]}=1;
		push(@transcripts,$$in[$path[$i]][$t]);
	      }
	    }
	  }
	  if($i<$#path) {
	    my $nr=scalar(@{$$out[$path[$i]]});
	    for(my $t=0;$t<$nr;$t++) {
	      if(!(defined $istranscript{$$out[$path[$i]][$t]}) && (($pathpat & $$no2tr[$$out[$path[$i]][$t]]) eq $$no2tr[$$out[$path[$i]][$t]])) { # transcript is compatible to path
		#print STDERR "Transcript ",$$out[$path[$i]][$t]," is compatible with path\n";
		$istranscript{$$out[$path[$i]][$t]}=1;
		push(@transcripts,$$out[$path[$i]][$t]);
	      }
	    }
	  }
	}
...
=begin
    ...
    for(my $i=0;$i<=$#transcripts;$i++) {
      print STDERR " ",$transcripts[$i],"(",$$intrcov[$transcripts[$i]],",",$$outtrcov[$transcripts[$i]],")";
    }
    print STDERR "\n";
...
=cut
	...
	my @nodeflux;
	my $flux=compute_flux(\@f,\@path,\@transcripts,$intrcov,$outtrcov,$oneperc,$trnode,$in,$out,\@nodeflux);
	...
	if($flux>$epsilon) {
	  ...
	  if($ve<$ne) { # if I didn't parse all edges at least once
	    ...
	    if($$nodecov[$maxi]>=$readthr) { # if I still have nodes that are above coverage threshold
	      ...
	      update_coverages($flux,\@path,\@nodeflux,$trnode,$tr2no,$in,$out,$intrcov,$outtrcov,\@transcripts,$trforder,$no2gnode,$oneperc);

=begin

	print STDERR "After update new cov of nodes are:\n";
	for(my $x=0;$x<$gno;$x++) {
	  print STDERR "Node[$x]=In:";
	  my $nin=scalar(@{$$in[$x]});
	  for(my $t=0;$t<$nin;$t++) {
	    print STDERR " ",$$in[$x][$t],"(",$$intrcov[$$in[$x][$t]],")";
	  }
	  print STDERR " Out:";
	  my $nout=scalar(@{$$out[$x]});
	  for(my $t=0;$t<$nout;$t++) {
	    print STDERR " ",$$out[$x][$t],"(",$$outtrcov[$$out[$x][$t]],")";
	  }	      
	  print STDERR "\n";
	}

	print STDERR "New vals for compatible transcripts are:";
	for(my $i=0;$i<=$#transcripts;$i++) {
	  print STDERR " ",$transcripts[$i],"(",$$intrcov[$transcripts[$i]],",",$$outtrcov[$transcripts[$i]],")";
	}
	print STDERR "\n";
...
=cut
	      ...
	      $geneno=parse_trf($ve,$ne,$maxi,$no2tr,$tr2no,$transfrag,$no2gnode,$compatible,$trnode,$in,$out,$oneperc,$nodecov,$intrcov,$outtrcov,$trforder,$seenedge,$refname,$geneno,$trno,$label,$strand,$readthr,$mintranscriptlen,$gno);
	    }
	  }
	}
      }
...
sub update_coverages {
  my ($flux,$path,$nodeflux,$trnode,$tr2no,$in,$out,$intrcov,$outtrcov,$transcripts,$trforder,$no2gnode,$onepercent)=@_;
...
=cut
  ...
  while($t<$nt && $nfilled<$n) { # as long as I didn't fill up the node quota
    ...
    # first I need to find how much of each transcript I need to use in order to fill up the quota
    my $trneed=0;
    for(my $i=0;$i<$nn;$i++) {
      my $substract=0;
      if($inquota[$$trnode[$sortedtrf[$t]][$i]] && ($nn==1 || $i)) { # I still have room to fill this node
	if($$intrcov[$sortedtrf[$t]]*$$onepercent[$$trnode[$sortedtrf[$t]][$i]]<$inquota[$$trnode[$sortedtrf[$t]][$i]]) {
	  $substract=$$intrcov[$sortedtrf[$t]];
	}
	else {
	  $substract=$inquota[$$trnode[$sortedtrf[$t]][$i]]/$$onepercent[$$trnode[$sortedtrf[$t]][$i]];
	}
	if($substract>$trneed) { $trneed=$substract;}
      }
      if($outquota[$$trnode[$sortedtrf[$t]][$i]] && ($nn==1 || $i<$nn-1)) { # I still have room to fill this node
	if($$outtrcov[$sortedtrf[$t]]*$$onepercent[$$trnode[$sortedtrf[$t]][$i]]<$outquota[$$trnode[$sortedtrf[$t]][$i]]) {
	  $substract=$$outtrcov[$sortedtrf[$t]];
	}
	else {
	  $substract=$outquota[$$trnode[$sortedtrf[$t]][$i]]/$$onepercent[$$trnode[$sortedtrf[$t]][$i]];
	}
	if($substract>$trneed) { $trneed=$substract;}
      }
    }

    #print STDERR "trneed=$trneed for transcript ",$sortedtrf[$t]," nn=$nn\n";
    
    # second update the nodes and their quotas if there was a need for the transcript
    if($trneed) {

      if($nn>1 || ($nn==1 && $inquota[$$trnode[$sortedtrf[$t]][0]])) { # update incoming transcript coverage
	$$intrcov[$sortedtrf[$t]]-=$trneed;
	#print STDERR "Updated in transcript ",$sortedtrf[$t]," to cov=",$$intrcov[$sortedtrf[$t]]," where trneed=$trneed\n";
      }

      if($nn>1 || ($nn==1 && $outquota[$$trnode[$sortedtrf[$t]][0]])) { # update outgoing transcript coverage
	$$outtrcov[$sortedtrf[$t]]-=$trneed;
	#print STDERR "Updated out transcript ",$sortedtrf[$t]," to cov=",$$outtrcov[$sortedtrf[$t]]," where trneed=$trneed\n";
      }

      for(my $i=0;$i<$nn;$i++) {
	# deal with the "in" transfrags
	if($i || ($nn==1 && $inquota[$$trnode[$sortedtrf[$t]][$i]]) ) {

	  #print STDERR "Check node ",$$trnode[$sortedtrf[$t]][$i]," with inquota=",$inquota[$$trnode[$sortedtrf[$t]][$i]]," and cov=",$$intrcov[$sortedtrf[$t]]," for nn=$nn\n";

	  my $incov=$inquota[$$trnode[$sortedtrf[$t]][$i]]/$$onepercent[$$trnode[$sortedtrf[$t]][$i]];
	  if($trneed>$incov) { # this transcript needs to be replaced by "nodes to nowhere"
	    my $excess=$trneed - $incov;
	    my $tnode=$$tr2no{$$no2gnode[$$trnode[$sortedtrf[$t]][$i]][8]};
	    if($$in[$$trnode[$sortedtrf[$t]][$i]][-1]!=$tnode) { # the node transcript could have been added only last
	      push(@{$$in[$$trnode[$sortedtrf[$t]][$i]]},$tnode);
	    }
	    $$intrcov[$tnode]+=$excess;
	    if($inquota[$$trnode[$sortedtrf[$t]][$i]] && !$outquota[$$trnode[$sortedtrf[$t]][$i]]) { $nfilled++;} # if the node's quota wasn't filled before
	    $inquota[$$trnode[$sortedtrf[$t]][$i]]=0;
	  }
	  else {
	    $inquota[$$trnode[$sortedtrf[$t]][$i]]-=$trneed*$$onepercent[$$trnode[$sortedtrf[$t]][$i]];
	    if($inquota[$$trnode[$sortedtrf[$t]][$i]]<$epsilon) { $inquota[$$trnode[$sortedtrf[$t]][$i]]=0;} # to prevent rounding errors
	    if(!$inquota[$$trnode[$sortedtrf[$t]][$i]] && !$outquota[$$trnode[$sortedtrf[$t]][$i]]) { 
	      $nfilled++; # if I filled the node's quota
	    }
	  }
	  if($$intrcov[$sortedtrf[$t]]<$epsilon) { # remove transcript from $in nodes; it's possible here that the node was not in the in nodes (if nn==1)
	    my $j=0;
	    my $nin=scalar(@{$$in[$$trnode[$sortedtrf[$t]][$i]]});
	    while($j<$nin && $$in[$$trnode[$sortedtrf[$t]][$i]][$j] != $sortedtrf[$t]) { $j++;}

	    if($j<$nin) {
	      #print STDERR "Remove transcript ",$sortedtrf[$t]," from in[",$$trnode[$sortedtrf[$t]][$i],"]\n";

	      splice @{$$in[$$trnode[$sortedtrf[$t]][$i]]},$j,1;
	    }
	  }
	}
	
	#print STDER "Done inquota\n";

	# deal with the "out" transfrags
	if($i<$nn-1 || ($nn==1 && $outquota[$$trnode[$sortedtrf[$t]][$i]])) {

	  #print STDERR "Check node ",$$trnode[$sortedtrf[$t]][$i]," with outquota=",$outquota[$$trnode[$sortedtrf[$t]][$i]]," and cov=",$$outtrcov[$sortedtrf[$t]]," for nn=$nn\n";
	  
	  my $outcov=$outquota[$$trnode[$sortedtrf[$t]][$i]]/$$onepercent[$$trnode[$sortedtrf[$t]][$i]];
	  if($trneed>$outcov) { # this transcript needs to be replaced by "nodes to nowhere"
	    my $excess=$trneed-$outcov;
	    my $tnode=$$tr2no{$$no2gnode[$$trnode[$sortedtrf[$t]][$i]][8]};
	    if($$out[$$trnode[$sortedtrf[$t]][$i]][-1]!=$tnode) { # the node transcript could have been added only last
	      push(@{$$out[$$trnode[$sortedtrf[$t]][$i]]},$tnode);
	    }
	    $$outtrcov[$tnode]+=$excess;
	    if(!$inquota[$$trnode[$sortedtrf[$t]][$i]] && $outquota[$$trnode[$sortedtrf[$t]][$i]]) { $nfilled++;} # if the node's quota wasn't filled before
	    $outquota[$$trnode[$sortedtrf[$t]][$i]]=0;
	  }
	  else {
	    $outquota[$$trnode[$sortedtrf[$t]][$i]]-=$trneed*$$onepercent[$$trnode[$sortedtrf[$t]][$i]];
	    if($outquota[$$trnode[$sortedtrf[$t]][$i]]<$epsilon) { $outquota[$$trnode[$sortedtrf[$t]][$i]]=0;} # to prevent rounding errors
	    if(!$inquota[$$trnode[$sortedtrf[$t]][$i]] && !$outquota[$$trnode[$sortedtrf[$t]][$i]]) { $nfilled++;} # if I filled up the node's quota here
	  }

	  if($$outtrcov[$sortedtrf[$t]]<$epsilon) { # remove transcript from $in nodes
	    my $j=0;
	    
	    #print STDERR "outquota[",$$trnode[$sortedtrf[$t]][$i],"]=",$outquota[$$trnode[$sortedtrf[$t]][$i]]," and cov=",$$outtrcov[$sortedtrf[$t]]," for nn=$nn\n";

	    my $nout=scalar(@{$$out[$$trnode[$sortedtrf[$t]][$i]]});
	    while($j<$nout && $$out[$$trnode[$sortedtrf[$t]][$i]][$j] != $sortedtrf[$t]) { $j++;} # this should always find it because otherwise it was already removed

	    if($j<$nout) {
	      #print STDERR "Remove transcript ",$sortedtrf[$t]," from out[",$$trnode[$sortedtrf[$t]][$i],"]\n";

	      splice @{$$out[$$trnode[$sortedtrf[$t]][$i]]},$j,1;
	    }
	  }
	}

	#print STDERR "inquota[",$$trnode[$sortedtrf[$t]][$i],"]=",$inquota[$$trnode[$sortedtrf[$t]][$i]]," and outquota[",$$trnode[$sortedtrf[$t]][$i],"]=",$outquota[$$trnode[$sortedtrf[$t]][$i]],"\n"; 

      }

      # to correct rounding errors
      if($$intrcov[$sortedtrf[$t]]<$epsilon) { 
	$$intrcov[$sortedtrf[$t]]=0; # to prevent rounding errors
      }
      if($$outtrcov[$sortedtrf[$t]]<$epsilon) { 
	$$outtrcov[$sortedtrf[$t]]=0; # to prevent rounding errors
      }


    }

    #print STDERR "nfilled=$nfilled t=$t nt=$nt\n";

    $t++;
  }

  # clean up one node transcripts
  for(my $i=0;$i<$n;$i++) {
    if($i) { # it's an in transcript
      if(defined $$in[$$path[$i]][-1]) { # there are transcripts going in
	if($$intrcov[$$in[$$path[$i]][-1]]<$epsilon) {
	  $$intrcov[$$in[$$path[$i]][-1]]=0;
	  pop(@{$$in[$$path[$i]]});
	}
      }
    }
    if($i<$n-1) { # it's an out transcript
      if(defined $$out[$$path[$i]][-1]) { # there are transcripts going in
	if($$outtrcov[$$out[$$path[$i]][-1]]<$epsilon) {
	  $$outtrcov[$$out[$$path[$i]][-1]]=0;
	  pop(@{$$out[$$path[$i]]});
	}
      }
    }
...
sub compute_flux {
  my ($f,$path,$transcripts,$intrcov,$outtrcov,$oneperc,$trnode,$in,$out,$nodeflux)=@_;
  ...
  for(my $t=0;$t<$n;$t++) {
    ...
    $istranscript{$$transcripts[$t]}=1;

    $$intrcov[$$transcripts[$t]]=0 unless $$intrcov[$$transcripts[$t]]>$epsilon;
    $$outtrcov[$$transcripts[$t]]=0 unless $$outtrcov[$$transcripts[$t]]>$epsilon;
    ...
    for(my $i=0;$i<$nn;$i++) {
      ...
      if($i || $nn==1) { $incapacity[$$trnode[$$transcripts[$t]][$i]]+=$$intrcov[$$transcripts[$t]];}
      if($i<$nn-1 || $nn==1) { 
	$outcapacity[$$trnode[$$transcripts[$t]][$i]]+=$$outtrcov[$$transcripts[$t]];
	if($$trnode[$$transcripts[$t]][$i]==29) {
	  #print STDERR "outtrcov[",$$transcripts[$t],"]=",$$outtrcov[$$transcripts[$t]]," outcap=",$outcapacity[$$trnode[$$transcripts[$t]][$i]],"\n";
	}
  ...
  while($update==1) {
    ...
    # check if there are not covered nodes
    for(my $i=0;$i<$n;$i++) {
      if($i) { # path enters and exists this node
	if(!$incapacity[$$path[$i]]) {
	  if($outcapacity[$$path[$i]]) { # transcripts exit node when none enters it
	    my $nr=scalar(@{$$out[$$path[$i]]});
	    for(my $t=0;$t<$nr;$t++) {
	      if($istranscript{$$out[$$path[$i]][$t]}) { # need to remove transcript
		my $nn=scalar(@{$$trnode[$$out[$$path[$i]][$t]]});
		for(my $j=0;$j<$nn;$j++) {
		  if($j || $nn==1) { 
		    $incapacity[$$trnode[$$out[$$path[$i]][$t]][$j]]-=$$intrcov[$$out[$$path[$i]][$t]];
		    $incapacity[$$trnode[$$out[$$path[$i]][$t]][$j]]=0 unless $incapacity[$$trnode[$$out[$$path[$i]][$t]][$j]]>$epsilon;
		    if(!$incapacity[$$trnode[$$out[$$path[$i]][$t]][$j]] && $$intrcov[$$out[$$path[$i]][$t]] && $$trnode[$$out[$$path[$i]][$t]][$j]<$$path[$i]) {
		      $update++;
		    }
		  }
		  if($j<$nn-1 || $nn==1) { 
		    $outcapacity[$$trnode[$$out[$$path[$i]][$t]][$j]]-=$$outtrcov[$$out[$$path[$i]][$t]];
		    $outcapacity[$$trnode[$$out[$$path[$i]][$t]][$j]]=0 unless $outcapacity[$$trnode[$$out[$$path[$i]][$t]][$j]]>$epsilon;
		    if(!$outcapacity[$$trnode[$$out[$$path[$i]][$t]][$j]] && $$outtrcov[$$out[$$path[$i]][$t]] && $$trnode[$$out[$$path[$i]][$t]][$j]<$$path[$i]) {
		      $update++;
		    }
		  }
		}
		$istranscript{$$out[$$path[$i]][$t]}=0;
	      }
	    }
	  }
	}
      }
      if($i<$n-1) { # path exits this node
	if(!$outcapacity[$$path[$i]]) {
	  if($incapacity[$$path[$i]]) { # transcripts enters node when none exits it
	    my $nl=scalar(@{$$in[$$path[$i]]});
	    for(my $t=0;$t<$nl;$t++) {
	      if($istranscript{$$in[$$path[$i]][$t]}) { # need to remove transcript
		my $nn=scalar(@{$$trnode[$$in[$$path[$i]][$t]]});
		for(my $j=0;$j<$nn;$j++) {
		  if($j || $nn==1) { 
		    $incapacity[$$trnode[$$in[$$path[$i]][$t]][$j]]-=$$intrcov[$$in[$$path[$i]][$t]];
		    $incapacity[$$trnode[$$in[$$path[$i]][$t]][$j]]=0 unless $incapacity[$$trnode[$$in[$$path[$i]][$t]][$j]]>$epsilon;
		    if(!$incapacity[$$trnode[$$in[$$path[$i]][$t]][$j]] && $$intrcov[$$in[$$path[$i]][$t]]&& $$trnode[$$in[$$path[$i]][$t]][$j]<$$path[$i]) {
		      $update++;
		    }
		  }
		  if($j<$nn-1 || $nn==1) { 
		    $outcapacity[$$trnode[$$in[$$path[$i]][$t]][$j]]-=$$outtrcov[$$in[$$path[$i]][$t]];
		    $outcapacity[$$trnode[$$in[$$path[$i]][$t]][$j]]=0 unless $outcapacity[$$trnode[$$in[$$path[$i]][$t]][$j]]>$epsilon;
		    if(!$outcapacity[$$trnode[$$in[$$path[$i]][$t]][$j]] && $$outtrcov[$$in[$$path[$i]][$t]] && $$trnode[$$in[$$path[$i]][$t]][$j]<$$path[$i]) {
		      $update++;
		    }
		  }
		}
		$istranscript{$$in[$$path[$i]][$t]}=0;
	      }
	    }
	  }
	}
  ...
  if($flux) { # compute node coverages
    for(my $i=0;$i<$n;$i++) {
      ...
      #print STDERR "compute flux: flux[",$$path[$i],"]=f[$i]=",$$f[$i]," outcap=",$outcapacity[$$path[$i]],"\n";
...
sub fwd_to_sink_path {
  my ($i,$path,$pathincov,$pathoutcov,$pathpat,$transcripts,$istranscript,$in,$out,$intrcov,$outtrcov,$trnode,$compatible,$ve,$seenedge,$no2gnode,$tr2no,$no2tr,$gno,$computed)=@_;

  # find all children -> if no children then go back
  my $inode=$$no2gnode[$i];
    
  my $nc=scalar(@{$$no2gnode[$i][3]});
  #if($nc) { print STDERR "inode=$inode First child of $i is:",$$no2gnode[$i][3][0]," val=",$$no2gnode[$i][3][0][2]," nc=$nc\n";}
    
  if($nc) { # child is present

    my $maxc=0; # max child
    my $maxtrfno=0; # maximum number of transcripts
    my @maxtrf; # keeps transcripts to add for maxparent
	...
	if(!$$compatible[$te][$$transcripts[$j]]) { # edge to child and transcripts are not compatible -> don't consider this child
	  if(can_be_removed($$transcripts[$j],$trnode,$pathincov,$pathoutcov,$intrcov,$$path[0],$i)) {
	    $trfno-=$$intrcov[$$transcripts[$j]];
	    push(@rm,$j);
	...
	next; # go to next child
      ...
      my %seenout;
      # compute how many of the out[i] transcripts are compatible with the previous kept transcripts and with the edge to parent
      for(my $l=0;$l<$nr;$l++) {
	if(!$seen{$$out[$i][$l]}){
	  $seenout{$$out[$i][$l]}=1;
	  if($$compatible[$te][$$out[$i][$l]] && onpath($$out[$i][$l],$$pathpat,$$path[0],$i,$trnode,$no2gnode,$no2tr,$gno)) {
	    my $skip=0;
	    my $j=0;
	    while($j<$n && $$trnode[$$out[$i][$l]][0]<=$$trnode[$$transcripts[$j]][-1]) { # transcript could be incompatible to in node
	      if(!(defined $torm{$$transcripts[$j]})) { # transcript wasn't marked to be removed
		if(!$$compatible[$$out[$i][$l]][$$transcripts[$j]]) { # in transcript and transcripts are not compatible
		  $skip=1;
		  last;
		}
	      }
      ...
      for(my $t=0;$t<$nl;$t++) {
	if(!$seen{$$in[$c][$t]} && !$seenout{$$in[$c][$t]}) { # transcript was not added before
	  if($$compatible[$$in[$c][$t]][$te] && onpath($$in[$c][$t],$$pathpat,$$path[0],$i,$trnode,$no2gnode,$no2tr,$gno)) {
	    my $skip=0;
	    my $j=0;
	    while($j<$n && $$trnode[$$in[$c][$t]][0]<=$$trnode[$$transcripts[$j]][-1]) { # transcript could be incompatible to in node
	      if(!(defined $torm{$$transcripts[$j]})) { # transcript wasn't marked to be removed
		if(!$$compatible[$$in[$c][$t]][$$transcripts[$j]]) { # in transcript and transcripts are not compatible
		  $skip=1;
		  last;
		}
	      }
      ...
      my @keeptrf=();
      $trfno+=max_compon_size(\@trf,\@keeptrf,$compatible,\%cov,$computed);

=begin

      print STDERR "Result=";
      for(my $r=0;$r<=$#keeptrf;$r++) {
	print STDERR " ",$keeptrf[$r],"(",$cov{$keeptrf[$r]},")";
      }
      print STDERR "\n";
...
=cut
	...
	$maxc=$c;
	$maxtrfno=$trfno;
	@maxtrf=@keeptrf; # these could be sped up by remembering addresses instead of copying all the array
	@maxrm=@rm;
      }
    }
    ...
    for(my $t=$#maxrm;$t>=0;$t--) {
      ...
      for(my $i=0;$i<$nn;$i++) {
	if($i) { $$pathincov{$$trnode[$maxrm[$t]][$i]}-=$$intrcov[$maxrm[$t]];}
	if($i<$nn-1) { $$pathoutcov{$$trnode[$maxrm[$t]][$i]}-=$$outtrcov[$maxrm[$t]];}
      }
    }
    
    # add transcripts from trf to the set of kept transcripts
    for(my $t=0;$t<=$#maxtrf;$t++) {
      #print STDERR "Add transcript ",$maxtrf[$t]," to transcripts\n";
      add_tr2vec($maxtrf[$t],-1,-1,$trnode,$transcripts);
      $$istranscript{$maxtrf[$t]}=1;
      my $nn=scalar(@{$$trnode[$maxtrf[$t]]});
      for(my $i=0;$i<$nn;$i++) {
	if($i) { $$pathincov{$$trnode[$maxtrf[$t]][$i]}+=$$intrcov[$maxtrf[$t]];}
	if($i<$nn-1) { $$pathoutcov{$$trnode[$maxtrf[$t]][$i]}+=$$outtrcov[$maxtrf[$t]];}
      }
    }

    $ve=fwd_to_sink_path($maxc,$path,$pathincov,$pathoutcov,$pathpat,$transcripts,$istranscript,$in,$out,$intrcov,$outtrcov,$trnode,$compatible,$ve,$seenedge,$no2gnode,$tr2no,$no2tr,$gno,$computed);
  }
...
sub back_to_source_path {
  ...
  #print STDERR "First parent of $i is:",$$inode[4][0]," val=",$$inode[4][0][2],"\n";
...
#    my @maxtrfcomp; # keeps transcripts to add for maxparent
	...
	if(!$$compatible[$te][$$transcripts[$j]]) { # edge to parent and transcripts are not compatible -> don't consider this parent
	  if(can_be_removed($$transcripts[$j],$trnode,$pathincov,$pathoutcov,$intrcov,$i,$$path[-1])) {
	    $trfno-=$$intrcov[$$transcripts[$j]];
	...
	next;
      ...
      # compute how many of the in[i] transcripts are compatible with the previous kept transcripts and with edge to the parent
      my %seenin;
      for(my $l=0;$l<$nl;$l++) {
	if(!$seen{$$in[$i][$l]}){
	  $seenin{$$in[$i][$l]}=1;

	  if($$compatible[$te][$$in[$i][$l]] && onpath($$in[$i][$l],$$pathpat,$i,$$path[-1],$trnode,$no2gnode,$no2tr,$gno)) { # in transcript needs to be compatible to edge and with the path so far in order to be added to path
	    my $skip=0;
	    my $j=0;
	    while($j<$n && $$trnode[$$in[$i][$l]][-1]>=$$trnode[$$transcripts[$j]][0]) { # transcript could be incompatible to in node
	      if(!(defined $torm{$$transcripts[$j]})) { # transcript wasn't marked to be removed
		if(!$$compatible[$$in[$i][$l]][$$transcripts[$j]]) { # in transcript and transcripts are not compatible
		  $skip=1;
		  last;
		}
	      }
      ...
      for(my $t=0;$t<$nr;$t++) {
	if(!$seen{$$out[$p][$t]} && !$seenin{$$out[$p][$t]}) { # transcript was not added before

	  if($$compatible[$$out[$p][$t]][$te] && onpath($$out[$p][$t],$$pathpat,$i,$$path[-1],$trnode,$no2gnode,$no2tr,$gno)) { # transcript needs to be compatible with edge and with path so far
	    my $skip=0;
	    my $j=0;
	    while($j<$n && $$trnode[$$out[$p][$t]][-1]>=$$trnode[$$transcripts[$j]][0]) { # transcript could be incompatible to in node
	      if(!(defined $torm{$$transcripts[$j]})) { # transcript wasn't marked to be removed
		if(!$$compatible[$$out[$p][$t]][$$transcripts[$j]]) { # in transcript and transcripts are not compatible
		  $skip=1;
		  last;
		}
	      }
...
=cut
	...
	$maxp=$p;
	$maxtrfno=$trfno;
	@maxtrf=@keeptrf; # these could be speed up by remembering addresses instead of copying all the array
	@maxrm=@rm;
      }
    }
    ...
    for(my $t=$#maxrm;$t>=0;$t--) {
      ...
      # added this: update path coverage
      my $nn=scalar(@{$$trnode[$maxrm[$t]]});
      for(my $i=0;$i<$nn;$i++) {
	if($i) { $$pathincov{$$trnode[$maxrm[$t]][$i]}-=$$intrcov[$maxrm[$t]];}
	if($i<$nn-1) { $$pathoutcov{$$trnode[$maxrm[$t]][$i]}-=$$outtrcov[$maxrm[$t]];}
    ...
    for(my $t=0;$t<=$#maxtrf;$t++) {
      #print STDERR "Add transcript ",$maxtrf[$t]," to transcripts\n";
      add_tr2vec($maxtrf[$t],1,0,$trnode,$transcripts);
      $$istranscript{$maxtrf[$t]}=1;
    ...
    $ve=back_to_source_path($maxp,$path,$pathincov,$pathoutcov,$pathpat,$transcripts,$istranscript,$in,$out,$intrcov,$outtrcov,$trnode,$compatible,$ve,$seenedge,$no2gnode,$tr2no,$no2tr,$gno,$computed);
  }
...
sub can_be_removed {
  my ($t,$trnode,$pathincov,$pathoutcov,$cov,$mini,$maxi)=@_;
  ...
  for(my $i=0;$i<$n;$i++) {
    if($i && $$pathincov{$$trnode[$t][$i]}) { 
      if($$trnode[$t][$i]>$mini && $$trnode[$t][$i]<=$maxi) {
	if($$pathincov{$$trnode[$t][$i]}==$$cov[$t]) { 
	  #print STDERR "pathincov{",$$trnode[$t][$i],"}=cov[$t]=",$$cov[$t],"\n";
	  return(0);
	}
      }
    }
    if($i<$n-1 && $$pathoutcov{$$trnode[$t][$i]}) {
      if($$trnode[$t][$i]>=$mini && $$trnode[$t][$i]<$maxi) {
	if($$pathoutcov{$$trnode[$t][$i]}==$$cov[$t]) { 
	  #print STDERR "pathoutcov{",$$trnode[$t][$i],"}=cov[$t]=",$$cov[$t],"\n";
	  return(0);
	}
      }
    }
...
sub find_transfragnodes {
    my ($t,$gno,$no2tr,$trnode,$in,$out,$sumin,$sumout,$transfrag)=@_;

    #print STDERR "Find nodes for transfrag $t\n";

    # create trnode; 
    for(my $i=1;$i<$gno;$i++) {
	if(vec($no2tr,$i,1)) { # node appears in transfrag

	  #print STDERR "found in node $i\n";
	  push(@{$$trnode[$t]},$i);
	    
	  # insert transfrag into in node in order given by firstnode, if i is not the first node
	  if($i!=$$trnode[$t][0]) {
	    add_tr2vec($t,1,0,$trnode,$$in[$i]);
	    $$sumin[$i]+=$$transfrag[$t];
	    
	    #print STDERR "Add to node $i, transfrag[$t]=",$$transfrag[$t]," sumin[$i]=",$$sumin[$i],"\n";
	  }
	}
    }
    ...
    my $n=scalar(@{$$trnode[$t]});
    for(my $i=0;$i<$n-1;$i++) {
      #print STDERR "add to out node ",$$trnode[$t][$i]," ",$$transfrag[$t]," sumout=",$$sumout[$$trnode[$t][$i]],"\n";
      add_tr2vec($t,-1,-1,$trnode,$$out[$$trnode[$t][$i]]);
      $$sumout[$$trnode[$t][$i]]+=$$transfrag[$t];
    }
}
...
sub update_nodes {
    my ($s,$g,$trnumber,$no2tr,$tr2no,$transfrag,$no2gnode,$gno,$compatible,$trnode,$in,$out,$sumin,$sumout)=@_; 
	...
	if(!defined $$trnode[$t1]) { 
	    find_transfragnodes($t1,$gno,$$no2tr[$s][$g][$t1],$trnode,$in,$out,$sumin,$sumout,$transfrag);
	}
	...
	for(my $t2=$t1+1;$t2<$$trnumber[$s][$g];$t2++) {
		...
		find_transfragnodes($t2,$gno,$$no2tr[$s][$g][$t2],$trnode,$in,$out,$sumin,$sumout,$transfrag);
	    }   
...
sub get_fragment_pattern {
	...
	for(my $sno=0;$sno<3;$sno+=2) {
		...
		if(!$differentgraphs) {
		    ...
		    vec($conflictpattn,$firstnode[$sno],1)=0b1; # here I get negative offset sometimes; maybe firstnode is -1? why doesn't it happen all the time??
		}
...
=begin
          ...
          # in this case I don't include the deletion in the counts of the reads, but might create problems with reads not connected in the same transcript
...
=cut
		...
		my $nj=add_junction($leftend,$rightstart,$leftsupport,$support,$junction,$hashjunction,$junctionsupport,$strand,$nh);
