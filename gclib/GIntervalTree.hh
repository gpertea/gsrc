#ifndef GINTERVALTREE_HH_
#define GINTERVALTREE_HH_
...
public:
  ...
  public:
    ...
    Node(const T&, N, N);
    virtual ~Node();
    N low() const { return key; }
    N high() const { return high_; }
  ...
  IntervalTree();
  IntervalTree(FILE* f, GFLoadProc fLoadFunc=NULL); //create by deserialization from file

  ~IntervalTree();

  void Store(FILE* f, GFStoreProc fStoreFunc=NULL); //write the tree into a file
  void Load(FILE* f, GFLoadProc fLoadFunc=NULL);
  //std::string str() const;
  void remove(N, N, GVec<T>&);
  template <class F> void remove(N, N, const F&, GVec<T>&);
  void remove_window(N, N, GVec<T>&);
  template <class F> void remove_window(N, N, const F&, GVec<T>&);
  Node * insert(const T&, N, N);
  void fetch(N, N, GVec<T>&);
  void fetch_window(N, N, GVec<T>&);
protected:
  void fetch_node(N, N, GVec<Node*>&);
  void fetch_window_node(N, N, GVec<Node*>&);
  T remove(Node *);
  ...
  bool CheckMaxHighFieldsHelper(Node * y, 
      ...
      bool match) const;
private:
  GVec<IntervalTree<T,N>::it_recursion_node> recursionNodeStack;
  int currentParent;
};
...
template<typename T, typename N> IntervalTree<T,N>::Node::Node()
  : //value_ (value__),
    key(numeric_limits<N>::min()),
    high_(numeric_limits<N>::min()),
    maxHigh(numeric_limits<N>::min()),
...
IntervalTree<T,N>::IntervalTree() : nodes(64, true)
{
  ...
  nil->color = BLACK;
  nil->key = nil->high_ = nil->maxHigh = numeric_limits<N>::min();
  ...
  root->key = root->high_ = root->maxHigh = numeric_limits<N>::max();
  root->color=BLACK;
...
template<typename T, typename N>
IntervalTree<T,N>::IntervalTree(FILE* f, GFLoadProc* fLoadFunc): nodes(64, true)
{
  nil = new IntervalTree<T,N>::Node();
  nil->left = nil->right = nil->parent = nil;
  nil->color = BLACK;
  ...
  nodes.Add(nil); //index 0
  nil->n_idx = nil->left_n_idx = nil->right_n_idx = nil->parent_n_idx = 0;
  root = new IntervalTree<T,N>::Node(); //root is index 1
  root->parent = root->left = root->right = nil;
  root->parent_n_idx = root->left_n_idx = root->right_n_idx = 0;
  root->n_idx = nodes.Add(root);
  root->key = root->high_ = root->maxHigh = numeric_limits<N>::max();
...
template<typename T, typename N>
void IntervalTree<T,N>::Load(FILE* f, GFLoadProc* fLoadFunc) {
...
void IntervalTree<T,N>::LeftRotate(IntervalTree<T,N>::Node* x) {
  ...
  x->maxHigh=GMAX(x->left->maxHigh, (GMAX(x->right->maxHigh,x->high_)));
  y->maxHigh=GMAX(x->maxHigh, (GMAX(y->right->maxHigh,y->high_)) );
#ifdef CHECK_INTERVAL_TREE_ASSUMPTIONS
  check();
#elif defined(DEBUG_ASSERT)
  if (nil->maxHigh!=numeric_limits<N>::min())
      GError("Error: nil->maxHigh != numeric_limits<N>::min() in ITLeftRotate!\n");
#endif
}
...
void IntervalTree<T,N>::RightRotate(IntervalTree<T,N>::Node* y) {
  ...
  y->maxHigh=GMAX(y->left->maxHigh,(GMAX(y->right->maxHigh,y->high_)));
  x->maxHigh=GMAX(x->left->maxHigh,(GMAX(y->maxHigh,x->high_)));
#ifdef CHECK_INTERVAL_TREE_ASSUMPTIONS
  check();
#elif defined(DEBUG_ASSERT)
  if (nil->maxHigh!=numeric_limits<N>::min())
        GError("Error: nil->maxHigh != numeric_limits<N>::min() in ITRightRotate!\n");
#endif
}
...
void IntervalTree<T,N>::TreeInsertHelp(IntervalTree<T,N>::Node* z) {
  ...
  if (nil->maxHigh!=numeric_limits<N>::min())
        GError("Error: nil->maxHigh != numeric_limits<N>::min() in ITTreeInsertHelp!\n");
#endif
}
...
void IntervalTree<T,N>::FixUpMaxHigh(IntervalTree<T,N>::Node * x) {
  while(x != root) {
    x->maxHigh=GMAX(x->high_, (GMAX(x->left->maxHigh,x->right->maxHigh)));
    x=x->parent;
  }
#ifdef CHECK_INTERVAL_TREE_ASSUMPTIONS
  check();
...
typename IntervalTree<T,N>::Node* IntervalTree<T,N>::insert(const T& newInterval, N low, N high)
{
  ...
  check();
#elif defined(DEBUG_ASSERT)
  if (root->color != RED) GError("Error: root not red in ITTreeInsert\n");
  if (nil->maxHigh!=numeric_limits<N>::min())
         GError("Error: nil->maxHigh != numeric_limits<N>::min() in ITTreeInsert\n");
#endif
}
...
void IntervalTree<T,N>::DeleteFixUp(IntervalTree<T,N>::Node* x) {
  ...
  check();
#elif defined(DEBUG_ASSERT)
  if (nil->color != BLACK) GError("Error: nil not black in ITDeleteFixUp\n");
  if (nil->maxHigh!=numeric_limits<N>::min())
         GError("Error: nil->maxHigh != numeric_limits<N>::min() in ITDeleteFixUp\n");
#endif
}
...
T IntervalTree<T,N>::remove(IntervalTree<T,N>::Node * z){
  ...
  if (y != z) { /* y should not be nil in this case */
...
#ifdef DEBUG_ASSERT
    if (y!=nil) GError("Error: y is nil in remove()!\n");
#endif
    /* y is the node to splice out and x is its child */
  
    y->maxHigh = numeric_limits<N>::min();
    y->left=z->left;
    y->right=z->right;
    y->parent=z->parent;
    ...
    check();
#elif defined(DEBUG_ASSERT)
    if (nil->color != BLACK) GError("Error: nil not black in ITDelete\n!");
    if (nil->maxHigh!=numeric_limits<N>::min())
        GError("Error: nil->maxHigh != numeric_limits<N>::min() in ITDelete\n");
#endif
  } else {
    FixUpMaxHigh(x->parent);
...
template <typename T, typename N>
void IntervalTree<T,N>::remove(N low, N high, GVec<T> &removed)
{
  GVec<IntervalTree<T,N>::Node*> got;
...
void IntervalTree<T,N>::remove(N low, N high, const F &removeFunctor, GVec<T> &removed)
{
  GVec<IntervalTree<T,N>::Node*> got;
  fetch_node(low, high, got);
  for (typename GVec<IntervalTree<T,N>::Node*>::const_iterator
...
template <typename T, typename N>
void IntervalTree<T,N>::remove_window(N low, N high, GVec<T> &removed)
{
  GVec<IntervalTree<T,N>::Node*> got;
...
template<typename T, typename N>
void IntervalTree<T,N>::fetch(N low, N high, GVec<T> &enumResultStack)  {
  IntervalTree<T,N>::Node* x=root->left;
...
#ifdef DEBUG_ASSERT
  if (recursionNodeStack.Count() == 0)
         GError("Error: recursionStack empty when entering IntervalTree::fetch\n");
#endif
  currentParent = 0;
...
#ifdef DEBUG_ASSERT
  if (recursionNodeStack.Count() == 0)
         GError("Error: recursionStack empty when exiting IntervalTree::fetch!\n");
#endif
}
...
void IntervalTree<T,N>::fetch_node(
    N low, 
    N high, 
    GVec<typename IntervalTree<T,N>::Node*> &enumResultStack)
{
  IntervalTree<T,N>::Node* x=root->left;
  bool stuffToDo = (x != nil);
...
#ifdef DEBUG_ASSERT
//  if (recursionNodeStack.Count() == 1)
//         GError("Error: recursionStack not empty when exiting IntervalTree::fetch\n");
#endif
}
        
template<typename T, typename N>
void IntervalTree<T,N>::fetch_window(N low, N high, GVec<T> &enumResultStack)
{
  IntervalTree<T,N>::Node* x=root->left;
  bool stuffToDo = (x != nil);
...
#ifdef DEBUG_ASSERT
//  if (recursionNodeStack.Count() == 1)
//         GError("Error: recursionStack not empty when entering IntervalTree::fetch_window\n");
#endif
  currentParent = 0;
...
#ifdef DEBUG_ASSERT
  if (recursionNodeStack.Count() == 0)
         GError("Error: recursionStack empty when entering IntervalTree::fetch_window_node\n");
#endif
  currentParent = 0;
...
void IntervalTree<T,N>::CheckMaxHighFields(IntervalTree<T,N>::Node * x) const {
  if (x != nil) {
    ...
    if(!(CheckMaxHighFieldsHelper(x,x->maxHigh,false) > 0)) {
      //assert(0);
      GError("Error: CheckMaxHighFields error\n");
    }
