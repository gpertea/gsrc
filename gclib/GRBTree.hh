...
struct rbnode_map_t {
    rbnode_map_t<KEY, OBJ> *rb_link[2];
    KEY key;
	OBJ value;
	signed char color;
};

template<class KEY, class __rbnode_t, class CMP> class GRBTree {
	CMP __cmp;
public:
	...
	GRBTree() : root(0), count(0), generation(0) {};
	~GRBTree() {
		node_t *p, *q;
	...
	inline node_t *find(const KEY &item) {
		node_t *p;
		for (p = root; p;) {
	...
	inline node_t *insert(const KEY &item, int *is_present) {
	...
	inline node_t *erase(const KEY &item) {
...
template<class KEY, class CMP> struct GRBSet : public GRBTree<KEY, rbnode_set_t<KEY>, CMP> {};

template<class KEY, class OBJ, class CMP> struct GRBMap : public GRBTree<KEY, rbnode_map_t<KEY, OBJ>, CMP> {};
